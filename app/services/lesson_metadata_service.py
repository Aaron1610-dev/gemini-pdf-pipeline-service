...
def _find_lesson_pdf(job_id: str, lesson_num: Any) -> Path:
    ...
    lesson_folder = bundle_dir / "Lesson" / f"lesson_{pad2(lesson_num)}"
    candidates = sorted(lesson_folder.glob("*.pdf")) if lesson_folder.exists() else []
    if not candidates:
        candidates = sorted((bundle_dir / "Lesson").glob(f"**/*lesson_{pad2(lesson_num)}*.pdf"))
    if not candidates:
        raise FileNotFoundError(f"Lesson PDF not found for lesson_{pad2(lesson_num)}.")
    return candidates[0]
