...
def _find_topic_pdf(job_id: str, topic_num: Any) -> Path:
    ...
    topic_folder = bundle_dir / "Topic" / f"topic_{pad2(topic_num)}"
    candidates = sorted(topic_folder.glob("*.pdf")) if topic_folder.exists() else []
    if not candidates:
        candidates = sorted((bundle_dir / "Topic").glob(f"**/*topic_{pad2(topic_num)}*.pdf"))
    if not candidates:
